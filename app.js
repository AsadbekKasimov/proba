...
async function fetchProducts() {
    const res = await fetch("https://script.google.com/macros/s/AKfycbyLPVc-tTaeJT4maw4jhMsKBCI22iP6P_GG0SuDqrhM5bCYYhGiPog0pqCS2JK2DXOKIw/exec");
    ...
    items.forEach(cartItem => {
        ...
        el.innerHTML = `
            ...
            <div class="order-detail-info">
                ...
                <div class="order-detail-price">${formatPrice(price)} × ${qty} = ${formatPrice(itemTotal)}</div>
            </div>
        `;
        itemsEl.appendChild(el);
    });

    let totalHTML = `<div class="order-total-row"><span>Итого:</span><span>${formatPrice(order.total)}</span></div>`;
